<script>
    ...
    $: {
        try{
            document.getElementById("ToolBarContentEditableDiv").innerHTML = $topic.currentTopic;
        }catch(e){  
        }
    }
        ...
        topic.update((data) => {
            recSearchReturnParent(
                ...
                (d, parent) => {
                    if (d.children) {
                        alert(
                            `Please delete all the subtopics of ${d.name} before deleting .`,
                        );
                        return;
                    }
                    var consent = confirm(
                        `Are you sure you want to delete ${d.name} topic from ${parent.name}?`,
                    );
                    if (consent) {
...
<div>
    <Toolbar>
        ...
        <div class="ml-2">
            <Button
                on:click={deleteCurrentNoteBook}
                color="alternative"
                class="!p-2 w-[35px] h-[35 px]"
